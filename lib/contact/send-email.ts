...
export async function sendContactEmail(
  ...
  const from = process.env.CONTACT_FROM_EMAIL?.trim() || smtp.auth.user;

  const safeName = sanitizeMailHeader(input.name);
  const safeEmail = sanitizeMailHeader(input.email);
  const subject = sanitizeMailHeader(`Portfolio inquiry from ${safeName}`);
  ...
  try {
    ...
    return { ok: false, reason: "Email provider failed." };
  }
